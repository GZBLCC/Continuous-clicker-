    ...
    def _process_message(self, message):
        ...
        if action == 'start':
            ...
            return {
                ...
                'random_area': self.clicker.random_area
            }
            
        elif action == 'set_random_area':
            area = message.get('area')
            if isinstance(area, list) and len(area) == 4:
                self.clicker.random_area = tuple(area)
                return {'status': 'random_area_set', 'area': self.clicker.random_area}
